...
public class HelpDeskSystem extends JFrame{
	...
	/**
	 * Returns the tickets if needed<br>
	 * 
	 ...
	 */
	protected Technician getTechByID(int techID) throws java.lang.NullPointerException
	{
		...
		JOptionPane.showMessageDialog(null, "Technician "+techID+" is not in the system yet");
		return null;
	}
	 ...
	 */
	protected  Ticket getTicketByID(int ticketID) throws java.lang.NullPointerException
	{
		...
		JOptionPane.showMessageDialog(null, "Ticket "+ticketID+" is not in the system yet");
		return null;
	}
	...
	protected void initializeCompsForLogon(Employee loggedInEmp, LogonRole role)
	{
			...
			public void stateChanged(ChangeEvent e) {
	            ...
	            if (logonRole == LogonRole.USER )
	            {
	            	...
	            	if(pane.getSelectedIndex() == 1)
	            	{
	            		...
	            		hwSubmissionTab.resetInitialValues();
	            	}
	            }
		...
		{
			//Tab to search for tickets
			//Have the same functionalities as a technician
			//Adding - Removing - Editing - reAssigning etc ...
			searchSubmittedTicketsTab = new TicketSearchPanel(logonEmployee, logonRole);
		    mainSystemPanel.addTab("View Created Tickets", searchSubmittedTicketsTab);
		    
		    //Tab for searching for technicians and making them active/inactive
		    UserProfilePanel searchTechPanel = new UserProfilePanel (logonEmployee, logonRole,true);
		    mainSystemPanel.addTab("Search Tab", searchTechPanel);
		    
		    //Tab for adding a new technician
		    UserProfilePanel addTechPanel = new UserProfilePanel (logonEmployee, logonRole,true,true);
		    mainSystemPanel.addTab("Add Technician", addTechPanel);
		    
		    //Tab for adding a new supervisor
		    UserProfilePanel addSuperPanel = new UserProfilePanel (logonEmployee, logonRole,true,true,true);
		    mainSystemPanel.addTab("Add Supervisor", addSuperPanel);
		}
		
		else //For technicians
		{
			//Technician can only see and edit tickets
			//They have no tabs for users profiles
		    searchSubmittedTicketsTab = new TicketSearchPanel(logonEmployee, logonRole);
		    mainSystemPanel.addTab("View Created Tickets", searchSubmittedTicketsTab);
		}
